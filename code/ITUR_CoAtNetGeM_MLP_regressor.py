...
BEST_MODEL_PATH = os.path.join(OUT_DIR, "best_coatnet1_gemmlp_regressor_full.pth")
...
def main():
    ...
    ).to_csv(os.path.join(OUT_DIR, "regression_test_metrics_coatnet1_gemmlp_full.csv"), index=False)

    pd.DataFrame({"y_true": test_trues, "y_pred": test_preds}).to_csv(
        os.path.join(OUT_DIR, "regression_test_predictions_coatnet1_gemmlp_full.csv"),
        index=False
    )

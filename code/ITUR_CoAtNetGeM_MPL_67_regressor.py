...
BEST_MODEL_PATH = os.path.join(OUT_DIR, "best_coatnet1_gemmlp_regressor_full_67ch.pth")
...
IN_CHANS      = 67             # 64 AlphaEarth + 3 extra variables
...
class CoAtNetRegressor(nn.Module):
    def __init__(self, backbone_name=BACKBONE_NAME, in_chans=IN_CHANS, hidden_dim=512):
        super().__init__()
...
class AlphaEarthTIFRegDataset(Dataset):
    ...
    Each sample:
      ...
      - Read extra features: CAR_SER_VI, DILOCCON50_log_norm, P_USOSUEPV_norm
      - Broadcast these 3 scalars as constant channels over HxW
      - Concatenate -> 67 channels
    ...
    def __getitem__(self, i):
        ...
        img = per_channel_minmax(img)  # (64,H,W)

        # ---- 3 extra variables as constant channels (like Swin script) ----
        # CAR_SER_VI, DILOCCON50_log_norm, P_USOSUEPV_norm must be in the CSV
        ...
        c, h, w = img.shape
        # Broadcast extras to match spatial dims: (3,H,W)
        extras_layer = np.repeat(extras[:, None, None], h, axis=1)
        extras_layer = np.repeat(extras_layer, w, axis=2)

        # Concatenate to form 67 channels
        img_full = np.concatenate([img, extras_layer], axis=0)  # (67,H,W)

        # Resize to 224x224 for CoAtNet
        img_t = torch.from_numpy(img_full)  # (67,H,W)
        img_t = F.interpolate(
...
def main():
    ...
    # Need the extra variables, analogous to Keras script
    ...
    # CoAtNet + GeM + 2-layer MLP regression head
    model = CoAtNetRegressor(backbone_name=BACKBONE_NAME, in_chans=IN_CHANS)
    ...
    # To save training history like Keras history.history
    history = []
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xb, yb in tqdm(train_loader, desc=f"Train {epoch}/{EPOCHS}", leave=False):
            ...
            loss.backward()
            optimizer.step()
            scheduler.step()  # OneCycle-like schedule as in the Keras script
        ...
        # Early stopping on validation loss (Keras-style)
    ...
    # Save training history CSV (like training_history_swin67.csv)
    hist_df = pd.DataFrame(history)
    hist_path = os.path.join(OUT_DIR, "training_history_coatnet1_gemmlp_67ch.csv")
    ...
    ).to_csv(os.path.join(OUT_DIR, "regression_test_metrics_coatnet1_gemmlp_67ch.csv"), index=False)

    # Save predictions
    pd.DataFrame({"y_true": test_trues, "y_pred": test_preds}).to_csv(
        os.path.join(OUT_DIR, "regression_test_predictions_coatnet1_gemmlp_67ch.csv"),
        index=False
    )

    torch.save(model.state_dict(), BEST_MODEL_PATH)
    print(f"Saved test metrics and predictions to {OUT_DIR}")
    print(f"Best CoAtNet1 GeM+MLP regressor weights saved to {BEST_MODEL_PATH}")

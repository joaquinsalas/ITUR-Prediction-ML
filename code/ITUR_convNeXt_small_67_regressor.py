...
BEST_MODEL_PATH = os.path.join(OUT_DIR, "best_convnext_small_gemmlp_regressor_full_67ch.pth")
...
IMG_SIZE      = 224            # ConvNeXt standard input
# ConvNeXt-Small pretrained variant (timm)
...
# ----------------------------
# ConvNeXt-Small regressor with GeM + 2-layer MLP head
# ----------------------------
...
class AlphaEarthTIFRegDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # ---- 3 extra variables as constant channels ----
...
def main():
    ...
    # ConvNeXt-Small + GeM + 2-layer MLP regression head
    model = ConvNeXtRegressor(backbone_name=BACKBONE_NAME, in_chans=IN_CHANS)
    ...
    hist_path = os.path.join(OUT_DIR, "training_history_convnext_small_gemmlp_67ch.csv")
    ...
    ).to_csv(os.path.join(OUT_DIR, "regression_test_metrics_convnext_small_gemmlp_67ch.csv"), index=False)

    # Save predictions
    pd.DataFrame({"y_true": test_trues, "y_pred": test_preds}).to_csv(
        os.path.join(OUT_DIR, "regression_test_predictions_convnext_small_gemmlp_67ch.csv"),
        index=False
    )

    torch.save(model.state_dict(), BEST_MODEL_PATH)
    print(f"Saved test metrics and predictions to {OUT_DIR}")
    print(f"Best ConvNeXt-Small GeM+MLP regressor weights saved to {BEST_MODEL_PATH}")

...
BEST_MODEL_PATH = os.path.join(OUT_DIR, "best_coatnet1_gemmlp_regressor_CAR_SER_VI_64ch.pth")
...
def main():
    ...
    # Need COD_CELDA and CAR_SER_VI at minimum
    needed = {"COD_CELDA", "CAR_SER_VI"}
    ...
    hist_path = os.path.join(OUT_DIR, "training_history_coatnet1_gemmlp_CAR_SER_VI_64ch.csv")
    ...
    print(f"\nTest R² (CAR_SER_VI): {test_r2:.4f} | Test MSE: {test_mse:.6f}")

    # Save metrics
    pd.DataFrame(
        [{"metric": "test_R2", "value": float(test_r2)},
         {"metric": "test_MSE", "value": float(test_mse)}]
    ).to_csv(os.path.join(OUT_DIR, "regression_test_metrics_coatnet1_gemmlp_CAR_SER_VI_64ch.csv"), index=False)

    # Save predictions
    pd.DataFrame({"y_true": test_trues, "y_pred": test_preds}).to_csv(
        os.path.join(OUT_DIR, "regression_test_predictions_coatnet1_gemmlp_CAR_SER_VI_64ch.csv"),
        index=False
    )

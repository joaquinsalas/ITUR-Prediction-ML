...
BEST_MODEL_PATH = os.path.join(OUT_DIR, "best_resnet50_regressor_full_geq_tau.pth")

TAU          = 0.878389      # lower bound for ITUR
SEED         = 42
BATCH_SIZE   = 256
...
# constrained output helper
def bounded_output(raw, low=TAU, high=1.0):
    """
    raw: (B,) unconstrained logits
    returns: y in [low, high]
    """
...
def eval_regressor(model, loader, device, criterion):
    ...
    for xb, yb in loader:
        ...
        # raw output then map to [TAU, 1]
        raw = model(xb).squeeze(-1)
        yhat = bounded_output(raw, low=TAU, high=1.0)
...
def main():
    ...
    # keep only samples with RESUL_ITUR >= TAU
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xb, yb in tqdm(train_loader, desc=f"Train {epoch}/{EPOCHS}", leave=False):
            ...
            # raw output then bounded to [TAU, 1]
            raw = model(xb).squeeze(-1)
            pred = bounded_output(raw, low=TAU, high=1.0)
    ...
    ).to_csv(os.path.join(OUT_DIR, "regression_test_metrics_resnet50_full_geq_tau.csv"), index=False)

    pd.DataFrame({"y_true": test_trues, "y_pred": test_preds}).to_csv(
        os.path.join(OUT_DIR, "regression_test_predictions_resnet50_full_geq_tau.csv"), index=False
    )

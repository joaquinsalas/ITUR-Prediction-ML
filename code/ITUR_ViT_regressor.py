...
BEST_MODEL_PATH = os.path.join(OUT_DIR, "best_vit_regressor_full.pth")
...
class AlphaEarthTIFRegDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # simple per-image, per-channel min-max normalization
...
def main():
    ...
    # ViT regression head
    model = timm.create_model(VIT_NAME, in_chans=IN_CHANS, pretrained=True)
    if hasattr(model, "head"):
        in_features = model.head.in_features
        model.head = nn.Linear(in_features, 1)
    elif hasattr(model, "head_drop") and hasattr(model, "fc_norm"):
        # some timm ViT variants
    ...
    ).to_csv(os.path.join(OUT_DIR, "regression_test_metrics_vit_full.csv"), index=False)

    pd.DataFrame({"y_true": test_trues, "y_pred": test_preds}).to_csv(
        os.path.join(OUT_DIR, "regression_test_predictions_vit_full.csv"), index=False
    )

    torch.save(model.state_dict(), BEST_MODEL_PATH)
    print(f"Saved test metrics and predictions to {OUT_DIR}")
    print(f"Best ViT regressor weights saved to {BEST_MODEL_PATH}")

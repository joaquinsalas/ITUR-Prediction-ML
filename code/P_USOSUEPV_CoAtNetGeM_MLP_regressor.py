...
BEST_MODEL_PATH = os.path.join(OUT_DIR, "best_coatnet1_gemmlp_regressor_P_USOSUEPV_64ch.pth")
...
class AlphaEarthTIFRegDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # Target: P_USOSUEPV_norm
        y = float(row["P_USOSUEPV_norm"])
...
def main():
    ...
    # Need COD_CELDA and P_USOSUEPV_norm at minimum
    needed = {"COD_CELDA", "P_USOSUEPV_norm"}
    ...
    hist_path = os.path.join(OUT_DIR, "training_history_coatnet1_gemmlp_P_USOSUEPV_64ch.csv")
    ...
    print(f"\nTest R² (P_USOSUEPV_norm): {test_r2:.4f} | Test MSE: {test_mse:.6f}")

    # Save metrics
    pd.DataFrame(
        [{"metric": "test_R2", "value": float(test_r2)},
         {"metric": "test_MSE", "value": float(test_mse)}]
    ).to_csv(os.path.join(OUT_DIR, "regression_test_metrics_coatnet1_gemmlp_P_USOSUEPV_64ch.csv"), index=False)

    # Save predictions
    pd.DataFrame({"y_true": test_trues, "y_pred": test_preds}).to_csv(
        os.path.join(OUT_DIR, "regression_test_predictions_coatnet1_gemmlp_P_USOSUEPV_64ch.csv"),
        index=False
    )

...
BEST_MODEL_PATH = os.path.join(OUT_DIR, "best_mlp_mixer_gem_regressor.pth")

SEED         = 42
BATCH_SIZE   = 128
EPOCHS       = 10000           # early stopping will cut it
LR           = 1e-4
MAX_LR       = 1e-3
WEIGHT_DECAY = 1e-2
PRINT_EVERY  = 1
...
# Model / input
IN_CHANS     = 64              # AlphaEarth embeddings depth
IMG_SIZE     = 224             # we resize 47x47 -> 224x224
PATCH_SIZE   = 7               # 224/7 = 32 tokens per side (N=1024)
HIDDEN_DIM   = 512             # mixer channel dimension
TOK_MLP_DIM  = 256             # token-mixing MLP width
CH_MLP_DIM   = 1024            # channel-mixing MLP width
NUM_BLOCKS   = 12              # mixer depth
DROPOUT      = 0.1
...
# ----------------------------
class MLP(nn.Module):
    def __init__(self, dim, hidden, dropout=DROPOUT):
        super().__init__()
...
class MixerBlock(nn.Module):
    def __init__(self, n_tokens, hidden_dim, tok_mlp_dim, ch_mlp_dim, dropout=DROPOUT):
        super().__init__()
        # x: (B, N, C)
        self.ln1 = nn.LayerNorm(hidden_dim)
        self.token_mlp = MLP(n_tokens, tok_mlp_dim, dropout)
        self.ln2 = nn.LayerNorm(hidden_dim)
        self.channel_mlp = MLP(hidden_dim, ch_mlp_dim, dropout)
    ...
    def forward(self, x):
        ...
        # Channel mixing
        y = self.ln2(x)
        y = self.channel_mlp(y)            # (B, N, C)
        x = x + y
        return x

class GeM(nn.Module):
    """Generalized Mean Pooling over tokens (N dimension)."""
...
class MLPMixerRegressor(nn.Module):
    def __init__(
        ...
        tok_mlp_dim=TOK_MLP_DIM,
        ch_mlp_dim=CH_MLP_DIM,
        ...
        # Patchify + linear projection to hidden_dim
        self.patch_embed = nn.Conv2d(in_chans, hidden_dim, kernel_size=patch_size, stride=patch_size, bias=True)

        # Mixer blocks
        blocks = []
        for _ in range(num_blocks):
            blocks.append(MixerBlock(n_tokens=n_tokens, hidden_dim=hidden_dim,
                                     tok_mlp_dim=tok_mlp_dim, ch_mlp_dim=ch_mlp_dim, dropout=dropout))
        self.blocks = nn.Sequential(*blocks)
        self.pre_ln = nn.LayerNorm(hidden_dim)

        # GeM pooling over tokens
        self.gem = GeM()
    ...
    def forward(self, x):
        # x: (B, C, H, W)
        x = self.patch_embed(x)                 # (B, hidden_dim, G, G)
        x = x.flatten(2).transpose(1, 2)        # (B, N, hidden_dim)
        x = self.blocks(x)                      # (B, N, hidden_dim)
        x = self.pre_ln(x)
        x = self.gem(x)                         # (B, hidden_dim)
...
def main():
    ...
    ).to_csv(os.path.join(OUT_DIR, "regression_test_metrics_mlp_mixer_gem.csv"), index=False)

    pd.DataFrame({"y_true": test_trues, "y_pred": test_preds}).to_csv(
        os.path.join(OUT_DIR, "regression_test_predictions_mlp_mixer_gem.csv"), index=False
    )

    torch.save(model.state_dict(), BEST_MODEL_PATH)
    print(f"Saved test metrics and predictions to {OUT_DIR}")
    print(f"Best MLP-Mixer+GeM regressor weights saved to {BEST_MODEL_PATH}")

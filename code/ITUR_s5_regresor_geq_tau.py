...
BEST_MODEL_PATH= os.path.join(OUT_DIR, "20251127best_s5_regressor.pt")
...
EPOCHS       = 100
LR           = 1e-4
MAX_LR       = 3e-3           # <-- Added for OneCycleLR
WEIGHT_DECAY = 1e-2
PRINT_EVERY  = 1
...
class S5Regressor(nn.Module):
    def __init__(
        ...
        self.head = nn.Linear(d_model, 1)  # unconstrained output
    ...
    def forward(self, x):
        ...
        # Map to [low, high]
        y = self.low + (self.high - self.low) * torch.sigmoid(raw)
        return y
...
def main():
    ...
    model = S5Regressor(low=TAU, high=1.0).to(DEVICE)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % PRINT_EVERY == 0:
            print(f"Epoch {epoch:03d} | Train MSE {tr_mse:.6f} | Val R2 {val_r2:.4f} | LR {scheduler.get_last_lr()[0]:.6f}")
    ...
    pd.DataFrame([{"metric": "test_R2", "value": float(test_r2)}]).to_csv(
        os.path.join(OUT_DIR, "regression_test_r2.csv"), index=False
    )
    pd.DataFrame({"y_true": test_trues, "y_pred": test_preds}).to_csv(
        os.path.join(OUT_DIR, "20251127regression_test_predictions_gtn_tau.csv"), index=False
    )
    torch.save(model.state_dict(), BEST_MODEL_PATH)

    print(f"Saved test R² and predictions to {OUT_DIR}")

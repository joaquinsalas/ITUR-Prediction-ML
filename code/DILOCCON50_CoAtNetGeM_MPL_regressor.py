...
BEST_MODEL_PATH = os.path.join(OUT_DIR, "best_coatnet1_gemmlp_regressor_DILOCCON50_64ch.pth")
...
IN_CHANS      = TIF_BANDS      # use ONLY the 64 AlphaEarth embeddings
...
class AlphaEarthTIFRegDataset(Dataset):
    ...
    Each sample:
      ...
      - Target: DILOCCON50_log_norm (scalar)
    """
    ...
    def __getitem__(self, i):
        ...
        # Target: DILOCCON50_log_norm
        y = float(row["DILOCCON50_log_norm"])
        ...
        # No extra scalar channels; use only 64 AlphaEarth channels
        img_full = img  # (64,H,W)

        # Resize to 224x224 for CoAtNet
        img_t = torch.from_numpy(img_full)  # (64,H,W)
        img_t = F.interpolate(
...
def main():
    ...
    # Need COD_CELDA and DILOCCON50_log_norm at minimum
    needed = {"COD_CELDA", "DILOCCON50_log_norm"}
    ...
    hist_path = os.path.join(OUT_DIR, "training_history_coatnet1_gemmlp_DILOCCON50_64ch.csv")
    ...
    print(f"\nTest R² (DILOCCON50_log_norm): {test_r2:.4f} | Test MSE: {test_mse:.6f}")

    # Save metrics
    pd.DataFrame(
        [{"metric": "test_R2", "value": float(test_r2)},
         {"metric": "test_MSE", "value": float(test_mse)}]
    ).to_csv(os.path.join(OUT_DIR, "regression_test_metrics_coatnet1_gemmlp_DILOCCON50_64ch.csv"), index=False)

    # Save predictions
    pd.DataFrame({"y_true": test_trues, "y_pred": test_preds}).to_csv(
        os.path.join(OUT_DIR, "regression_test_predictions_coatnet1_gemmlp_DILOCCON50_64ch.csv"),
        index=False
    )
